...
const formActionTarget = computed(() => activeEdit.value === null ? addComment : updateComment);
const formActionSubmitText = computed(() => activeEdit.value === null ? 'Add Comment' : 'Update Comment');
...
const updateComment = () => {
    const endpoint = route('comments.update', {
        comment: activeEdit.value,
        page: comments.meta.current_page,
    });

    commentForm.put(endpoint, {
        preserveScroll: true,
        onSuccess: () => (activeEdit.value = null),
    })
};
...
watch(activeEdit, async (comment) => {
...
<template lang="pug">
    AppLayout(:title="post.title")
        Container
            ...
            .bg-white.mx-8.mt-4.px-8.py-4.rounded-2xl(v-if="$page.props.auth.user && (comments.meta.current_page === 1 || activeEdit)")
                form.flex.gap-4.justify-between(@submit.prevent="formActionTarget")
                    UserAvatar.size-16.place-self-center(:user="$page.props.auth.user")
                    .w-full
                        InputLabel.sr-only(for="hostdoc-comment-body") Comment
                        TextArea.resize-none#hostdoc-comment-body(v-model="commentForm.body", rows="3", placeholder="Weigh in with your thoughts", ref="commentTextArea")
                    PrimaryButton.text-nowrap(type="submit", :disabled="commentForm.processing")
                        IconAddComment.size-5(v-if="!activeEdit")
                        | &nbsp;{{ formActionSubmitText }}
                    SecondaryButton(v-if="activeEdit" @click="activeEdit = null")
                        | Cancel
                ...
                .relative
                    Dropdown(v-if="commentCreatedBy(comment.user)")
                        ...
                        template(v-slot:content)
                            ...
                            DropdownLink(as="button", @click="activeEdit = comment", :disabled="comment.can?.update !== true")
                                .flex.gap-4
                                    IconEdit.size-5
                                    span Edit
                            DropdownLink(as="button", @click="deleteComment(comment.id)", :disabled="comment.can?.destroy !== true || activeEdit?.id === comment.id")
                                .flex.gap-4
                                    IconTrashCan.size-5
                                    span Delete
